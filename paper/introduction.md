...
We present a session type system for intra-warp communication that statically eliminates diverged shuffle and ballot operations. Well-typed programs cannot perform unsafe warp operations on inactive lanes. The guarantee is zero-overhead—enforcement is purely compile-time. Global memory fence interactions under divergence are explicitly out of scope and identified as future work.
