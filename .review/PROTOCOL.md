...
### v3 → v4 changes (after round 3, --scope all, 2026-03-20)

| v3 Problem | v4 Fix |
|---|---|
| Flat dispatch only (3 agents, code-only) | Hierarchical dispatch: aspect agents per category, wave execution |
| No research domain categories | 6 research categories: claims, correspondence, formalism, prior-art, reproducibility, lean4 |
| No category relevance detection | Auto-detection with ACTIVE/SKIP/ASK per category |
| No toolchain freshness check | Repo checklist item 9: TOOLCHAIN FRESHNESS (90d/180d thresholds) |
| debug_assert for user-facing bounds (4 instances) | Upgraded to assert! in tile.rs, dynamic.rs, cub.rs |
| CpuSimd::shuffle_down wrapped instead of clamping | Fixed to GPU-style clamp semantics |
| paper.md ballot rule used Warp<S> (contradicted impl) | Fixed to Warp<All> |
| SimWarp shuffle_idx reported as wrong (wraps) | Hardware verified: wraps mod 32 is correct GPU behavior |
...
- Hardware verification overrides cold-agent claims about runtime behavior (shuffle_idx wrap was correct)
- Toolchain staleness is invisible to code-focused review — repo hygiene must check infrastructure age
