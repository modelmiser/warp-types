...
The `.reduced` type family is the architectural contribution of the Reduce instance (§5.7). It is the only constructor that extends the `CoreTy` inductive beyond the four constructors shared by all other domains (`.group`, `.data`, `.unit`, `.pair`). The extension is a *type-family* extension in the §5.7 taxonomy — cheap, requiring only a `TyTag` constructor and four dispatcher clauses — not a *rule* extension, which would require re-verification of existing inversion theorems.
