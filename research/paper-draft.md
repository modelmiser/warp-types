...
| Partial-write fence | `Fence.lean` | 271 | `.group` (`ByteBuf := PSet 8`) | `IsComplement` | `fence` requires `.group (PSet.all 8)` | `unit` (barrier) |
| Tree all-reduce | `Reduce.lean` | 302 | `.reduced` (`Col := PSet 4`) | `IsComplement` | `finalize` requires `.reduced (PSet.all 4)` | `data` (accumulator value) |
