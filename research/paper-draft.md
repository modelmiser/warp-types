...
The two gates are orthogonal to two further axes: the extract gate can return a barrier (Fence) or a value (Reduce), and the merge gate can act on different type families indexed by participant set — permissions (`.group`) or accumulators (`.reduced`). Stating the gates once, parametric in both axes, is the technical contribution. Two *family-parametric* typing rules — rules parameterized over the choice of `PSet n`-indexed type-family constructor — called `mergeFamily` and `finalizeFamily` are factored into a generic core file `Core.lean` and instantiated per domain via type-family tags, `@[reducible]` dispatchers, and a double equality witness on both the expression form and the conclusion type.

**Contributions.** **(1)** A generic core language for complemented typestate (`Core.lean`, 276 lines) whose two family-parametric rules subsume the merge and extract rules of Fence and Reduce, while the same complement gate governs all four domains at the `Generic.lean` level. **(2)** A zero-`sorry`, zero-user-declared-`axiom` Lean 4 mechanization, verified by `lake build WarpTypes`. **(3)** A structural extensibility win: adding a fifth domain costs one tag constructor and four dispatcher clauses, not a fresh typing judgment — though the refactor adds 108 lines net (§6). **(4)** An elaboration-layer finding: family-parametric rules in Lean 4 require a *double* equality witness (one on the expression, one on the output type), and the caller-side discharge cost is linear in the product of nested-cases depth and parametric-rule count (§6).
...
|-------|-------------|-------|------------------|-----------|-------------|-------------|
| GPU divergence | `Basic.lean` + `Metatheory.lean` | — | `.warp` (`ActiveSet := PSet 32`)¹ | `IsComplement` | `shuffle` requires `.warp (PSet.all 32)` | `perLane` (broadcast value)² |
...
¹ *`Basic.lean` pre-dates `Core.lean` and uses its own `Ty` inductive with `.warp` in the role that `.group` plays in the other three domains.*
² *`Basic.lean`'s shuffle returns `.perLane` (per-lane data); the other three domains' extract returns `.data` or `.unit`.*

All four rows use `PSet.IsComplement s1 s2 parent` at the merge site and `PSet.all n` at the extract site. Rows differ in (i) which `PSet n`-indexed type family carries the participant set (`.warp` for the GPU instance, `.group` for CSP and Fence, `.reduced` for Reduce); and (ii) whether the extract gate returns a barrier (Fence) or a data payload (the other three). The same two gates govern every row.
...
The paper assumes familiarity with dependent type theory at the level of Lean 4 or Coq, and with linear type systems in the sense of Girard's linear logic (1987) as presented in Wadler's *Linear Types Can Change the World*. This section fixes notation and names the three concepts that support the generic framework: complemented participant sets, linear typestate, and the gate terminology.
...
**Figure 6.** *The `WarpTypes` file graph.* Three files (`Generic.lean`, `Core.lean`, and `CoreMetatheory.lean`) make up the generic spine; five are domain instances, one carries the GPU-specific reduction metatheory and bug witnesses, and `Protocol.lean` extends CSP with protocol-compliance proofs (out of scope for this paper). The rightmost column pins the concrete width each file commits to; `—` means the file is parametric in `n`.
...
| `Protocol.lean` | `Csp` | CSP protocol-compliance proofs (out of scope) | n = 6 |
...
The metatheory follows the standard Wright–Felleisen (1994) approach: progress and preservation are proved independently, and type safety is their standard corollary (induction over multi-step reduction). The generic-versus-domain split in the file graph has a counterpart at the theorem level. Figure 7 tallies which theorem classes live at which layer, which are stated parametrically in `n`, and which carry untypability proofs for a real-world bug class.
...
Five untypability theorems model real GPU bugs at `n = 32`: <!-- unguarded: Five — per-file Lean theorem count; the guard totals only Basic + Metatheory -->
...
Four per-domain theorems: <!-- unguarded: Four — per-file Lean theorem count; the guard totals only Basic + Metatheory -->
...
The Reduce instance models a tree-structured all-reduce over four participants. `Col := PSet 4` is the smallest halving width with a non-trivial intermediate step. Reduce introduces a new type-family constructor `.reduced s` — the accumulator type — alongside the `.group s` constructor shared by CSP and Fence (and the structurally analogous `.warp s` in the GPU instance). The `leafReduce` rule is a cross-family coercion: it takes a `.group s` handle and produces a `.reduced s` accumulator. The `combineRed` rule is subsumed by `mergeFamily` at tag `.reduced`: it takes two accumulators of types `.reduced s1` and `.reduced s2`, requires `PSet.IsComplement s1 s2 parent`, and produces `.reduced parent`. The `finalize` rule is subsumed by `finalizeFamily` at tag `.reduced`: it requires `.reduced (PSet.all n)` and produces `.data`.

The `.reduced` type family is the architectural contribution of the Reduce instance (§5.7). It is the only constructor that extends the `CoreTy` inductive beyond the four constructors shared by the other Core-based domains (`.group`, `.data`, `.unit`, `.pair`). The extension is a *type-family* extension in the §5.7 taxonomy — cheap, requiring only a `TyTag` constructor and four dispatcher clauses — not a *rule* extension, which would require re-verification of existing inversion theorems.

Four per-domain theorems, mirroring Fence's: <!-- unguarded: Four — per-file Lean theorem count; the guard totals only Basic + Metatheory -->
...
The linearity discipline of the typing judgment — `Γ ⊢ e : τ ⊣ Γ'`, with bindings consumed exactly once — descends from the session-type tradition originating in Honda's *Types for Dyadic Interaction* (1993) and developed by Honda, Yoshida, and Carbone into the multiparty session type (MPST) framework (POPL 2008). The Caires–Pfenning correspondence (2010) established a Curry–Howard link between intuitionistic linear logic and session types, showing that linear propositions correspond to process types and cut elimination corresponds to communication. Wadler's *Propositions as Sessions* (2012) extended the correspondence to classical linear logic, giving a different proof-theoretic foundation for the same session-type discipline; the two correspondences are complementary, and the present framework's linear context threading is compatible with either foundation. The explicit context-threading discipline `Γ ⊢ e : τ ⊣ Γ'` used in the present framework follows the standard presentation in Walker's tutorial on substructural type systems (2005) and Gay and Vasconcelos's linear type theory for asynchronous sessions (2010).
...
Multiparty session types (Honda, Yoshida, and Carbone, POPL 2008; journal version JACM 2016) are the closest point of contact. MPST governs correctness of multi-party protocols by projecting a global type onto local types for each participant, with linearity ensuring that each message is consumed exactly once. The CSP instance shares the "multiple participants, one protocol" structure, but the complement gate — requiring that two sub-groups cover a parent before merging — has no MPST analog. MPST's safety comes from the projection being well-formed; complemented typestate's safety comes from the bitvector algebra being sound. The two are complementary verification strategies, not competitors.

### 7.2 Typestate and permission systems

Typestate was introduced by Strom and Yemini (1986) and developed into a practical type-system component by DeLine and Fähndrich (2004), who applied it to resource management in Vault and later to the Fugue protocol checker. Bierhoff and Aldrich (2007) extended typestate to aliased objects by introducing *access permissions* — a fine-grained ownership discipline that lets typestate checks survive in the presence of shared references. The present framework avoids the aliasing problem entirely via linearity (each handle is consumed exactly once), which is a simpler but less flexible strategy than Bierhoff–Aldrich's permission-based approach. The core idea — types that change as operations are applied — is the ancestor of the present framework's participant-set tracking. Each `diverge` changes the type of the warp handle from `.warp s` to a pair of handles at the two sub-sets; each `merge` changes the types of two sub-set handles back to a single parent handle. The participant-set index is the typestate, and the transition rules are the typing rules.
...
Prior work on GPU correctness has used verification rather than type systems. Betts, Chong, Donaldson, Qadeer, and Thomson (OOPSLA 2012) introduced GPUVerify, which uses barrier invariants and two-thread reduction to verify data-race freedom and barrier-divergence freedom for GPU kernels. GPUVerify specifically addresses the barrier-divergence bug class that the Fence instance models — its barrier invariants require all threads to reach a barrier, structurally analogous to the complement gate requiring all bytes to be written before a fence. GPUVerify and subsequent SMT-based GPU verification tools verify specific programs against specific assertions; complemented typestate rejects entire classes of programs by construction. The two approaches are complementary: SMT verification can prove properties (liveness, data-flow correctness) that a type system cannot, while the type system provides zero-cost compile-time rejection of the bug class that SMT must check at analysis time.

To our knowledge, no prior type system tracks active lane masks at the type level to prevent shuffle-from-inactive-lane bugs. The GPU instance (§4.1) is the first type-directed approach to this bug class. The present framework goes further in two directions: the participant-set index is width-parametric (`PSet n` at arbitrary `n`, not fixed at 32 lanes), so the same typing rules apply to AMD 64-lane wavefronts and to non-GPU domains; and the generic core (§5) factors the merge and extract gates into family-parametric rules, so the type system is no longer SIMT-specific.
...
All sections drafted and cold-reviewed (4 rounds, converged 2026-04-12 at 0 CRITICAL + 0 MODERATE + 0 LOW). The following items remain for submission:

- ~~**§1 word count**~~ — DONE. Trimmed from ~960 to ~813 words (contributions paragraph and "two axes" paragraph compressed).
- ~~**Citations**~~ — DONE. `research/references.bib` has 17 verified BibTeX entries. Two prior entries (Li-Donaldson 2017, Bao-Chen-Hovemeyer 2020) were removed after exhaustive verification confirmed they are nonexistent publications. §7.3 rewritten: the GPU type system is positioned as original work with no direct type-system ancestor.
- **§1 "three type-system traditions" claim** — already sharpened: "three different type-system traditions (session types for CSP, typestate for resource tracking, warp-correctness type systems for GPU), with partial-write fencing having no prior type-system treatment."
- **LaTeX conversion** — convert markdown to SIGPLAN two-column format, integrate `.bib`, add `\cite{}` keys.
...
- **Artifact grep note** — `grep -r sorry lean/` finds one hit in `CoreExperiment.lean`, but it is inside a block comment (`/- ... -/`) quoting a deliberately failing proof as a negative artifact — not executable code. The nine production files and `lake build WarpTypes` are sorry-free.
