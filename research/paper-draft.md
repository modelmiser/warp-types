...
The GPU instance is the origin of the framework and the most fully developed domain. `ActiveSet := PSet 32` pins one bit per lane of an NVIDIA warp. The expression language (`Expr n`) has 15 constructors: value forms for warps, per-lane data, and unit; a linear variable; `diverge`, `merge`, and `shuffle` as the three warp-collective operations; `let`, pair introduction and elimination; and four loop forms (`loopUniform`, `loopVarying`, `loopPhased`, `loopConvergent`) that track the warp handle through iterative control flow. The typing judgment `HasType` carries 18 rules, of which `diverge`, `merge`, and `shuffle` are the three that exercise the gates:
...
Six per-domain theorems cover the CSP instance:
