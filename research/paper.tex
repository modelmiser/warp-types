...
\textbf{(3)}~A structural extensibility win: adding a fifth domain costs one tag constructor and four dispatcher clauses, not a fresh typing judgment --- though the refactor adds 108~lines net at the port commit~(\S\ref{sec:evaluation}).

\textbf{(4)}~An elaboration-layer finding: family-parametric rules in Lean~4 require a \emph{double} equality witness (one on the expression, one on the output type), and the caller-side discharge cost is empirically linear in the product of nested-cases depth and parametric-rule count~(\S\ref{sec:evaluation}).
...
The metatheory follows the standard Wright--Felleisen~\cite{wright:ic:1994} approach: progress and preservation are proved independently, and type safety follows as a corollary (induction over multi-step reduction). The generic-versus-domain split in the file graph has a counterpart at the theorem level. Table~\ref{fig:theorem-placement} tallies which theorem classes live at which layer, which are stated parametrically in~$n$, and which carry untypability proofs for a real-world bug class.
...
All five are one-line applications of \texttt{shuffle\_diverged\_untypable}, which is itself a composition of \texttt{shuffle\_requires\_all} with a helper that walks the nested expression through \texttt{fst}, \texttt{diverge}, and \texttt{groupVal} --- the same helper pattern that~\S\ref{sec:evaluation} identifies as the source of the caller-side discharge cost. All five are corollaries of the same generic untypability theorem; the five instances are included to demonstrate correspondence with specific real-world bug reports, not to exercise different proof paths (bug1 and bug4 share the Lean term at mask \texttt{0x1}; bug2 and bug3 share the term at mask \texttt{0xFFFF}).

A positive instance, \texttt{merge\_then\_shuffle\_typable}, completes the positive/negative pair: merging the even-lane and odd-lane sub-warps via \texttt{even\_odd\_complement} reconverges the warp to $\texttt{PSet.all}\;32$, enabling a well-typed \texttt{shuffle} on the full participant set.
...
The per-domain theorem stack for CSP includes four inversion lemmas, one general untypability theorem, three concrete untypability witnesses, and one positive typability instance:
...
\begin{itemize}
  ...
  \item \texttt{j1\_collective\_after\_column\_split}: concrete untypability witness at $n = 6$ showing that a \texttt{collective} after splitting the grid into left and right columns is untypable. The CSP analog of the GPU-side bug witnesses \texttt{bug1}--\texttt{bug5} above.
\end{itemize}
...
Inside the \texttt{cases tag <;>} block, the dispatcher reduces to a concrete constructor (\texttt{.merge \_ \_} or \texttt{.combineRed \_ \_} in the merge case; \texttt{.fence \_} or \texttt{.finalize \_} in the extract case), and the equality witness \texttt{hExpr} is now of the form $\texttt{.fst}\;(\texttt{.diverge}\;\_\;\_) = \texttt{.merge}\;\_\;\_$ (or similar). This is a constructor clash, and \texttt{cases hExpr} closes the branch. The pattern is two lines in Lean~4 syntax (the pattern-match arm and the \texttt{cases tag} block), and \texttt{Fence.lean} and \texttt{Reduce.lean} collectively carry fourteen copies of it in the two helpers: three nested levels in \texttt{fence\_fst\_diverge\_groupval\_type} times two parametric rules, four nested levels in \texttt{reduce\_leaf\_fst\_diverge\_type} times two parametric rules. The inversion theorems (\texttt{fence\_requires\_all} and \texttt{finalize\_requires\_all}) each add two more discharge blocks, bringing the total to eighteen across both files. The two-line block is mechanical but it is not free: the pre-port Fence helper was ten lines, and the post-port version is twenty-two lines.
...
\texttt{Fence.lean} & 271 & 209 & $-62$ & 6 rule bodies removed; 14 lines of discharge added across 4 \texttt{cases} sites \\
\texttt{Reduce.lean} & 302 & 196 & $-106$ & 7 rule bodies removed; 26 lines of discharge added across 5 \texttt{cases} sites \\
\midrule
...
The discharge cost in the helpers is linear in the product of nested-cases depth and parametric-rule count. Call the number of parametric rules~$M$ and the total nesting depth across the helpers~$D$. Adding the~$M$-th parametric rule to a framework that already has~$D$ levels of nested cases costs $O(D)$ two-line discharge blocks, and these are not optional --- the port's \texttt{lake build} fails until every level is covered. The inversion theorems (\texttt{fence\_requires\_all}, \texttt{finalize\_requires\_all}) add a further $O(M)$ blocks each, but these are constant per theorem, not nested.

In the \texttt{Core.lean} case, $M = 2$ and $D = 7$ (three nesting levels in the Fence helper plus four in the Reduce helper), yielding $14$~discharge blocks in the helpers totalling $28$~lines. The inversion theorems add $4$~more blocks (${\sim}22$~additional lines), for a total of $18$~blocks and ${\sim}50$~lines across both files. This is manageable and visible in the diff. A framework with $M = 3$ parametric rules and $D \approx 15$ nested helper levels (roughly, one that serves five domains instead of two) would incur ${\sim}45$~helper discharges and ${\sim}90$~lines in the helpers alone. At that scale, either (a)~a custom \texttt{cases\_family} tactic that handles the pattern in one place, or (b)~a small-step inversion representation that avoids the nested-cases idiom entirely, becomes worth the investment. Neither is implemented in this work; both are identified as future work in~\S\ref{sec:conclusion}.

The practical consequence for a framework author contemplating a family-parametric refactor is: count the nested \texttt{cases} levels in the existing inversion theorems before you start, multiply by the number of parametric rules you intend to introduce, and use that product as the size of the boilerplate discharge you will have to carry. In the warp-types case the helper product was~14 (with 4~additional blocks in the inversion theorems), the port landed in one session, and the framework was preserved. For larger products, a tactic or a small-step encoding becomes mandatory.
...
The key distinction is in what linearity tracks. Session types track \emph{channel endpoints} --- a linear variable represents one end of a communication channel, and the type prescribes the sequence of send/receive operations. Complemented typestate tracks \emph{participant sets} --- a linear variable represents a permission handle on a set of participants, and the type prescribes which participants are active. The two are orthogonal: a session-typed channel could carry a complemented-typestate handle as a payload, and the CSP instance~(\S\ref{sec:csp-collective}) demonstrates that the topology-constrained \texttt{send}/\texttt{recv} rules coexist with the participant-set gates without interference. The framework does not subsume session types (it has no duality, no recursion, no subtyping on session types), and session types do not subsume the framework (they have no notion of participant-set complement at merge). Kobayashi's type systems for deadlock-free processes~\cite{kobayashi:concur:2006} are the closest point of overlap between the session-type cluster and the present work: Kobayashi tracks channel-use obligations in a way structurally closer to participant-set tracking than standard session types. The present framework does not address deadlock~(\S\ref{sec:conclusion} identifies this as a limitation), and Kobayashi's system does not address participant-set complements.

Multiparty session types~\cite{honda:popl:2008,honda:jacm:2016} are the closest point of contact. MPST governs correctness of multi-party protocols by projecting a global type onto local types for each participant, with linearity ensuring that each message is consumed exactly once. The CSP instance shares the ``multiple participants, one protocol'' structure, but the complement gate --- requiring that two sub-groups cover a parent before merging --- has no direct analog in MPST, including in Scalas and Yoshida's projection-free revisiting of the meta-theory~\cite{scalas:popl:2019} or Castro-Perez, Ferreira, and Yoshida's Coq mechanisation of MPST~\cite{castro-perez:tacas:2020} --- the closest mechanised point of comparison for the Lean development reported here. MPST's merge operator serves a structurally related but mechanically distinct role. MPST's safety comes from the projection being well-formed; complemented typestate's safety comes from the bitvector algebra being sound. The two are complementary verification strategies, not competitors.
...
Rust's ownership system (the borrow checker), formalised by Jung et al's RustBelt~\cite{jung:popl:2018} and Stacked Borrows~\cite{jung:popl:2020}, is a linearity discipline applied to memory safety, in the practical-affine-types tradition of Tov and Pucella~\cite{tov:popl:2011}. The analogy to complemented typestate is structural: Rust's \texttt{\&mut T} is a linear permission to mutate, and the borrow checker ensures that at most one mutable reference exists at a time. The difference is scope: Rust tracks individual memory locations, while complemented typestate tracks sets of participants. The Fence instance~(\S\ref{sec:partial-write-fence}) is the closest point of contact --- byte-granularity write permissions are analogous to byte-granularity borrow scopes --- but the complement gate (requiring that two byte-sets cover the full buffer before a fence commits) has no Rust analog. Rust's borrow checker does not reason about set complements.

\subsection{GPU correctness type systems}\label{sec:gpu-correctness}

Prior work on GPU correctness has used verification rather than type systems. Betts, Chong, Donaldson, Qadeer, and Thomson~\cite{betts:oopsla:2012} introduced GPUVerify, which uses barrier invariants and two-thread reduction to verify data-race freedom and barrier-divergence freedom for GPU kernels. Bardsley and Donaldson~\cite{bardsley:nfm:2014} later extended GPUVerify beyond barrier synchronisation to warp-level atomics and intra-warp communication, modelling the active-lane mask as an SMT variable that the verifier ranges over at each warp primitive --- the closest prior work to the GPU instance's bug class, but discharged per-program by an external solver rather than structurally by the type system. Sorensen and Donaldson~\cite{sorensen:pldi:2016} later addressed GPU weak-memory bugs, a distinct class. GPUVerify specifically addresses the barrier-divergence bug class that the Fence instance models --- its barrier invariants require all threads to reach a barrier, structurally analogous to the complement gate requiring all bytes to be written before a fence. GPUVerify and subsequent SMT-based GPU verification tools verify specific programs against specific assertions; complemented typestate rejects entire classes of programs by construction. The two approaches are complementary: SMT verification can prove properties (liveness, data-flow correctness) that a type system cannot, while the type system provides zero-cost compile-time rejection of the bug class that SMT must check at analysis time.

Formal-semantics work on SIMT execution has modelled the active-lane mask operationally rather than as a type-level index. Habermaier and Knapp~\cite{habermaier:esop:2012} give a small-step operational semantics for a SIMT kernel language in which the lane mask is a reduction-state component, and prove a correctness result relating lock-step SIMT execution to an interleaved thread semantics. Collingbourne, Donaldson, Ketema, and Qadeer~\cite{collingbourne:esop:2013} define dual interleaving and lock-step semantics for a GPU kernel language and establish an equivalence between them that underpins the GPUVerify two-thread reduction. Both bodies of work track active masks; they track them \emph{dynamically}, as components of a reduction state that a verifier or meta-theorem ranges over. The present framework tracks the same information \emph{statically}, as a type-level index on every expression, so that lane-mask divergence is detected by typing rather than by a whole-program verifier --- and so that the bug class is ruled out at compile time rather than caught by an external analysis pass.

Against that operational-semantics baseline, no prior \emph{type system} has tracked active lane masks as compile-time indices to prevent shuffle-from-inactive-lane bugs; to our knowledge, the GPU instance~(\S\ref{sec:gpu-divergence}) is the first type-directed approach to this bug class. The present framework goes further in two directions: the participant-set index is width-parametric ($\PSet\;n$ at arbitrary~$n$, not fixed at 32~lanes), so the same typing rules apply to AMD 64-lane wavefronts and to non-GPU domains; and the generic core~(\S\ref{sec:core}) factors the merge and extract gates into family-parametric rules, so the type system is no longer SIMT-specific.
...
Complemented typestate sits in a gap between these three traditions. It inherits linear context threading from session types, typestate transitions from the DeLine--F\"ahndrich tradition, and the specific bug-class motivation from GPU correctness work. What it contributes is the \emph{complement gate} --- a structural gate on the bitvector algebra of participant sets --- that is orthogonal to all three: session types have no notion of set complement, typestate systems have no notion of participant sets, and, to our knowledge, GPU correctness work has not previously used type-directed safety or been generalised across domains. The factoring of~\S\ref{sec:core}, which states the gate once in a family-parametric form and instantiates it at four domains, is the technical mechanism by which the generality is realised; the four domain instances of~\S\ref{sec:domains} are the evidence that the generalisation does not lose domain-specific theorem coverage.
...
The mechanization described in this paper is packaged for submission to the POPL 2027 Artifact Evaluation Committee. The artifact contains the Lean~4 library that realises \texttt{Generic.lean}, \texttt{Core.lean}, \texttt{CoreMetatheory.lean}, and the four domain instances (\texttt{Basic.lean}/\texttt{Metatheory.lean}, \texttt{Csp.lean}, \texttt{Fence.lean}, \texttt{Reduce.lean}) referenced throughout the paper, together with a Rust host-side workspace from which the kernel compilation pipeline described in~\S\ref{sec:gpu-divergence} is driven.

\paragraph{Toolchain pins.}
The Lean library pins v4.28.0 via a checked-in \texttt{lean-toolchain} file and takes no Mathlib dependency. The Rust workspace pins a dated nightly via \texttt{rust-toolchain.toml}; the nightly is required only for the \texttt{nvptx64} compile-to-PTX backend and is not on the proof path.

\paragraph{Build and run.}
From a clean clone, with \texttt{elan} and \texttt{rustup} installed, the full verification is \texttt{lake build WarpTypes} followed by \texttt{cargo test --workspace --lib}; both commands complete offline against the pinned toolchains. The Lean build emits only cosmetic \texttt{unused simp arg} warnings. The Rust test suite runs 512 unit tests across four crates (317 in the main \texttt{warp-types} crate, and 195 across the \texttt{warp-types-sat}, \texttt{warp-types-smt}, and \texttt{warp-types-pdr} sibling crates); \texttt{cargo test -p warp-types --doc} adds 28 main-crate doc tests. The compile-to-PTX pipeline requires GPU hardware and is validated out-of-band via \texttt{reproduce/runpod-h200.sh} (NVIDIA) and \texttt{reproduce/runpod-mi300x.sh} (AMD); it is excluded from the Docker-image verification path. Running \texttt{\#print axioms} on \texttt{type\_safety}, \texttt{progress}, and \texttt{preservation} reports only the three Lean~4 kernel axioms (\texttt{propext}, \texttt{Quot.sound}, \texttt{Classical.choice}); no user-declared axioms extend the foundation.

\paragraph{Library layout.}
The \texttt{WarpTypes.lean} module root imports ten files: \texttt{Generic.lean} (width-parametric layer); \texttt{Basic.lean} and \texttt{Metatheory.lean} (GPU instance, 32~lanes, standalone metatheory pre-dating the factoring); \texttt{Csp.lean} and \texttt{Protocol.lean} (CSP collective with J1 topology, \(n=6\)); \texttt{Core.lean} and \texttt{CoreMetatheory.lean} (family-parametric spine); \texttt{Fence.lean} (\(n=8\)) and \texttt{Reduce.lean} (\(n=4\)), both inheriting the Core metatheory via one-line specialisations; and \texttt{SolExperiment.lean} (a bitvector probe carried alongside the spine for the Sol experiment referenced in~\S\ref{sec:generic-lean}). Two further files, \texttt{CoreExperiment.lean} and \texttt{CspCoreExperiment.lean}, are not imported by the build target and carry exploratory probes only (\S\ref{sec:csp-collective}).

\paragraph{Claim-to-witness map.}
Each formal claim in the paper maps to a named \texttt{theorem} or \texttt{def} declaration that a reviewer can grep from the top of the indicated file.

\begin{itemize}\small
\item \textit{Zero user-declared axioms on \texttt{progress}, \texttt{preservation}, \texttt{type\_safety}} (\S\ref{sec:spine},~\S\ref{sec:core}) --- \texttt{\#print axioms} on the three theorems in \texttt{Metatheory.lean} and \texttt{CoreMetatheory.lean} reports only the Lean kernel axioms.
\item \textit{Five GPU bug witnesses at \(n=32\)} (\S\ref{sec:gpu-divergence}) --- \texttt{bug1\_*}\,--\,\texttt{bug5\_*} theorems in \texttt{Metatheory.lean}: four modeling real-world issue-tracker bugs, one synthetic.
\item \textit{Three concrete CSP untypability witnesses, one general untypability theorem, and four inversion lemmas} (\S\ref{sec:csp-collective}) --- three \texttt{j1\_*} untypability theorems (opposite-corners send, diagonal send, collective after column split); the general \texttt{collective\_after\_diverge\_untypable} theorem from which the \texttt{j1\_*} witnesses specialise; and the four \texttt{csp\_*} inversion lemmas (\texttt{csp\_diverge\_partition} plus three \texttt{csp\_*\_requires\_*} theorems) grouped above them in \texttt{Csp.lean}.
\item \textit{Partial-write untypability} (\S\ref{sec:partial-write-fence}) --- untypability witness at \(n=8\) in \texttt{Fence.lean}.
\item \textit{Partial-reduce untypability} (\S\ref{sec:tree-all-reduce}) --- untypability witness at \(n=4\) in \texttt{Reduce.lean}.
\item \textit{\texttt{mergeFamily} and \texttt{finalizeFamily} family-parametric rules} (\S\ref{sec:mergefamily},~\S\ref{sec:finalizefamily}) --- \texttt{CoreHasType} constructors in \texttt{Core.lean}, dispatched over \texttt{TyTag}.
\item \textit{Fence and Reduce inherit progress, preservation, and type safety via one-line specialisations} (\S\ref{sec:post-refactor}) --- specialised \texttt{theorem} blocks at the tails of \texttt{Fence.lean} and \texttt{Reduce.lean}.
\item \textit{\texttt{Core.lean} is 276 lines} (\S\ref{sec:core}) --- reproducible via \texttt{wc -l} on the file.
\item \textit{Caller-side cost of the factoring: +108 lines net, 18 discharge blocks (14 helpers, 4 inversion)} (\S\ref{sec:evaluation}) --- counts reproducible from the repository's tagged pre- and post-refactor snapshots.
\end{itemize}

\paragraph{Availability.}
The artifact will be made available via the POPL 2027 AEC submission portal upon conditional acceptance; a DOI-stamped, identity-bearing copy will be linked from the de-anonymised camera-ready version. The exact commit used to build this version of the paper is pinned in the submission package's manifest.
