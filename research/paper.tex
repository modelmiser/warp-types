...
From a clean clone, with \texttt{elan} and \texttt{rustup} installed, the full verification is \texttt{lake build WarpTypes} followed by \texttt{cargo test --workspace}; both commands complete offline against the pinned toolchains. The Lean build emits only cosmetic \texttt{unused simp arg} warnings. The Rust test suite runs 395 tests in total (317~unit, 50~example, 28~doc). Running \texttt{\#print axioms} on \texttt{type\_safety}, \texttt{progress}, and \texttt{preservation} reports only the three Lean~4 kernel axioms (\texttt{propext}, \texttt{Quot.sound}, \texttt{Classical.choice}); no user-declared axioms extend the foundation.
