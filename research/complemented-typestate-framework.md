...
**Status (2026-04-06):** Shipped as `lean/WarpTypes/Csp.lean` — 376 lines, 13 theorems, 0 sorry, 0 axioms. <!-- unguarded: dated research-log count; not a claim about current inventory -->
...
**Conservativity of Level 1 type-family extension.** A second-order finding from Experiment C: adding `reduced (s : PSet n)` as a new constructor to the Level 1 `ReduceTy` inductive broke zero existing theorems. `reduce_diverge_partition` still delegates to `diverge_partition_generic` unchanged; all 9 core typing rules copy-renamed from `Fence.lean` without modification; the context / lookup / remove infrastructure needed no changes because it is polymorphic in the type family by construction (`List (String × ReduceTy n)`). <!-- unguarded: dated research-log count; not a claim about current inventory -->
