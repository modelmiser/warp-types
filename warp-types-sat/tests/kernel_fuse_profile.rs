...
use warp_types_sat::gpu_gradient::{
    gradient_search_gpu, gradient_search_gpu_resident, gradient_soa, total_loss_simwarp,
...
fn gpu_resident_matches_original() {
    ...
    for seed in 0..10u64 {
        ...
        if original.assignment.is_some() {
            orig_sat += 1;
        }
        if resident.assignment.is_some() {
            res_sat += 1;
        }
...
fn gpu_variable_update_matches_cpu() {
    ...
    let (gpu_loss, gpu_norm_sq) = ctx
        .gpu_iteration(&gpu_data, &mut state, lr, momentum)
...
fn kernel_fuse_profile() {
    ...
    for &num_vars in &[500u32, 2000, 5000] {
        ...
        println!("--- {num_vars} vars, {num_clauses} clauses ---");
        println!("  Original (1 kernel + host xfer + CPU norm): {orig_median:>6.0} μs");
        println!(
            "  Resident (3 kernels, device-only):           {res_median:>6.0} μs  ({var_batches} var batches)"
        );
        println!("  Speedup: {:.2}x", orig_median / res_median);
    }
}
